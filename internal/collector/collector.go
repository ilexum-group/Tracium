...
func getProcesses() []models.ProcessInfo {
	...
	case "linux":
		...
		for _, entry := range entries {
			...
			// Limit to first 100 processes to avoid huge data
			if len(processes) >= 100 {
				break
			}
		}
	case "darwin":
		cmd := exec.Command("ps", "-eo", "pid,user,comm,%cpu,rss")
		output, err := cmd.Output()
		if err == nil {
			scanner := bufio.NewScanner(bytes.NewReader(output))
			scanner.Scan() // Skip header
	...
	case "freebsd", "openbsd":
		cmd := exec.Command("ps", "-axo", "pid,user,comm,%cpu,rss")
		output, err := cmd.Output()
		if err == nil {
			scanner := bufio.NewScanner(bytes.NewReader(output))
			scanner.Scan() // Skip header
		...
		if err == nil {
			...
			for scanner.Scan() && count < 50 {
				...
				if len(fields) >= 4 {
					...
					if fields[2] == "active" {
						status = "running"
					} else if fields[2] == "inactive" || fields[2] == "failed" {
						status = "stopped"
					}
